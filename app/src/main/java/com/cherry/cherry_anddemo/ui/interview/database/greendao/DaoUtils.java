...
public class DaoUtils {
    ...
    public synchronized static <T> boolean insertOrReplaceList(Class clazz, List<T> list){
        boolean flag = false;
        if (list != null && clazz != null && !list.isEmpty()){
     ...
     * @return
     */
    public synchronized static <T> boolean deleteWhere(Class clazz, WhereCondition cond, WhereCondition... condMore){
        boolean flag = false;
        if (clazz != null && cond != null){
            DaoManager.getInstance().getDaoSession().getDao(clazz).queryBuilder()
                    .where(cond,condMore).buildDelete().forCurrentThread().executeDeleteWithoutDetachingEntities();
            flag = true;
        }
        return flag;
    }
     ...
     * @return
     */
    public synchronized static <T> boolean deleteWhereOr(Class clazz, WhereCondition cond1, WhereCondition cond2, WhereCondition... condMore) {
        boolean flag = false;
        ...
        try {
            ...
            if (clazz != null){
                if (queryBuilder == null){
                    queryBuilder = DaoManager.getInstance().getDaoSession()
                            .queryBuilder(clazz);
                }
    ...
    public static void closeConnection() {
        DaoManager.getInstance().closeConnect();
    }
    /**
     * 实体类更新时调用
     */
